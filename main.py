...
# --- 📢 FIVE CHANNEL SOURCE CONFIGURATION ---
...
# --- 📁 FIVE DATABASE PATH CONFIGURATIONS ---
...
async def on_start_command(event):
    ...
    if reg_status and referrer_id:
        try: await client.send_message(int(referrer_id), f"🔔 *Referral Event!*\n\nA new user registered via your node.\n➕5 Token Limits credited!")
        except Exception: pass
...
async def send_advanced_dashboard(chat_id, user_id, message_id=None):
    ...
    welcome_ui = (
        f"💎 *QUAD-ENGINE INTERLINKED STREAM CORE v3.0* 💎\n"
        f"━━━━━━━⚙️ Account Framework ⚙️━━━━━━━\n"
        f"🎖️ **Tier Status** :  👑 `{user_data['plan']}`\n"
        f"📊 **Usage Token**:  ⚡ `{user_data['searches_today']}` / `{user_data['max_limit']}` Searches\n"
        f"⏳ **Limit Resets**:  🔄 Daily Sequence Control\n"
        f"━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n"
        f"💬 **Instructions**: Send any media title down below to scan files across multiple structural data servers."
    )

    keyboard = [
        [Button.inline("🔍 Scan Database Catalog", b"discover_hub")],
        [Button.inline("👤 Profile Metrics", b"account_status"), Button.inline("🎁 Free Daily Token", b"daily_reward")],
        [Button.inline("📈 Global Trends", b"stats_panel"), Button.inline("🏆 Referrers Board", b"leaderboard_view")],
        [Button.inline("💎 Upgrade Premium Tiers (UPI / Stars)", b"premium_menu")]
    ]
    
    if message_id:
        try: await client.edit_message(chat_id, message_id, welcome_ui, buttons=keyboard, parse_mode='markdown')
        except Exception: await client.send_message(chat_id, welcome_ui, buttons=keyboard, parse_mode='markdown')
    else:
        await client.send_message(chat_id, welcome_ui, buttons=keyboard, parse_mode='markdown')
...
async def on_interactive_callback(event):
    ...
    if action == b'discover_hub':
        await event.answer("💡 Just type your movie title directly into the chat container!", alert=True)
        ...
        if user_data['last_reward_time']:
            try:
                ...
                if now - last_claim < timedelta(hours=24):
                    ...
                    await event.answer(f"🔒 Quota Locked! Check back in {hours} hours.", alert=True)
            except Exception: pass
            
        if can_claim:
            bonus = random.randint(1, 5)
            await DatabaseManager.update_user_reward(user_id, bonus, now.strftime("%Y-%m-%d %H:%M:%S"))
            await event.answer(f"🎁 Bonus Granted! Added +{bonus} search tokens permanently to your ceiling!", alert=True)
        ...
        profile_ui = (
            ...
            f"🏅 **System Rank Tier**    : `★ {user_data['plan']} Tier Account`\n"
            f"⚡ **Token Performance**   : `{user_data['searches_today']}` / `{user_data['max_limit']}` Tokens Used\n"
            f"🤝 **Affiliate Count**    : `{user_data['referral_count']}` Node connections\n"
            f"⏳ **Subscription Cycle**  : `{user_data['premium_expiry']}`"
        )
        await event.edit(profile_ui, buttons=[[Button.inline("🔙 Return to Console", b"back_to_root")]], parse_mode='markdown')

    elif action == b'stats_panel':
        ref_link = f"https://t.me/{bot_identity.username}?start={user_id}"
        trending = await DatabaseManager.get_trending_movies()
        trends_ui = "🔥 *HIGHLY REQUESTED DATABASE METRICS*:\n"
        if trending:
            for idx, tr in enumerate(trending, 1):
                trends_ui += f" 🔹 `[{idx}]` {tr['file_name']} (Dispatched `{tr['search_count']}` times)\n"
        else:
            trends_ui += " _No global catalog metrics tracked yet._\n"

        stats_ui = (
            f"📈 **NETWORK ARCHITECTURE STATS**\n"
            f"━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n"
            f"🔗 **Affiliate Track Vector Link**:\n`{ref_link}`\n\n" + trends_ui
        )
        await event.edit(stats_ui, buttons=[[Button.inline("🔙 Return to Console", b"back_to_root")]], parse_mode='markdown')

    elif action == b'leaderboard_view':
        referrers = await DatabaseManager.get_top_referrers()
        leaderboard_ui = f"🏆 *GLOBAL AFFILIATE SYSTEM LEADERBOARD*\n━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n"
        for rank, ref in enumerate(referrers, 1):
            leaderboard_ui += f" 🌟 `[Rank {rank}]` 🏅 @{ref['username'] or 'Hidden Node'} with `{ref['referral_count']}` invites\n"
        await event.edit(leaderboard_ui, buttons=[[Button.inline("🔙 Return to Console", b"back_to_root")]], parse_mode='markdown')
        ...
        buttons = [
            ...
            [Button.inline("🔙 Return to Console", b"back_to_root")]
        ]
        await event.edit(upgrade_ui, buttons=buttons, parse_mode='markdown')
        ...
        try:
            ...
            await client.send_file(
                ...
                buttons=[[Button.inline("❌ Cancel Transaction Order", b"premium_menu")]]
            )
...
async def RenderPaginationView(event, query, matches, page=1):
    ...
    catalog_ui = (
        f"📂 *MULTIPLE CROSS-CHANNEL INDEX SERVERS ACTIVE* (5-DB集群)\n"
        f"━━━━━━━🔍 Catalog Matches 🔍━━━━━━━\n"
...
async def core_search_router(event):
    ...
    if user_data['searches_today'] >= user_data['max_limit']:
        ...
        await event.reply(over_limit_ui, buttons=[[Button.inline("💎 Open Premium Tiers", b"premium_menu")]], parse_mode='markdown')
        return
...
async def admin_central_terminal_cmd(event):
    ...
    panel = (
        f"👑 *SYSTEM MANAGEMENT EXECUTIVE LAYER v3.0* (`/adminGC`)\n"
        f"━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n"
